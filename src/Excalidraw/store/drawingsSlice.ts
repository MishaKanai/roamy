...
const drawingsSlice = createSlice({
  ...
  reducers: {
    replaceDrawings: {
      reducer(
        state,
        { payload: { drawings } }: PayloadAction<{ drawings: DrawingDocuments }>
      ) {
        return drawings;
      },
    ...
    createDrawing: {
      reducer(
        state,
        {
          payload: { drawing, drawingName, withBackref, createdDate },
        }: PayloadAction<{
          drawingName: string;
          drawing: DrawingData;
          withBackref?: string;
    ...
    builder
      .addCase(
        ...
        (
          ...
          {
            payload: {
              newDrawing: { files, ..._newDrawing },
              drawingName,
              updatedDate,
            },
          }: PayloadAction<{
            drawingName: string;
            newDrawing: Partial<DrawingData>;
            updatedDate: Date;
          }>
        ) => {
          const {
      ...
      .addCase(createDoc, (state, { payload: { docName, doc } }) => {
        ...
        Object.keys(state).forEach((drawingKey) => {
          if (
            referencesSet.has(drawingKey) &&
            !state[drawingKey].backReferences.includes(docName)
          ) {
      ...
      .addCase(
        updateDoc,
        (state, { payload: { docName, newDoc, prevDoc } }) => {
          ...
          let referencesDropped = prevReferences.filter(
            (x) => !referencesSet.has(x)
          );
          let referencesAdded = Array.from(referencesSet).filter(
            (x) => !prevReferencesSet.has(x)
          );
          Object.keys(state).forEach((drawingKey) => {
            const backReferences = state[drawingKey].backReferences.filter(
              (bref) => (bref === docName ? referencesSet.has(bref) : true)
            );
            if (referencesSet.has(drawingKey)) {
              if (!backReferences.includes(docName)) {
