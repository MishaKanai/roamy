...
const isTranscodingPlaceholder = (
  node: Descendant
): node is TranscodingPlaceholderElement => {
  return (node as CustomElement).type === "transcoding_placeholder";
};

export const addRemoteFileCount = (
...
export const getRemoteFilesFromNodes = (nodes: Descendant[]): RemoteFiles => {
  ...
  nodes.forEach((n) => {
    ...
    if ((n as any).children) {
      Object.entries(
        getRemoteFilesFromNodes((n as any).children as Descendant[])
      ).forEach(([fileId, { count }]) => {
...
const getReferencesFromNodes = (nodes: Descendant[]): Set<string> => {
  ...
  nodes.forEach((n) => {
    ...
    if ((n as any).children) {
      getReferencesFromNodes((n as any).children as Descendant[]).forEach(
        (r) => {
          references.add(r);
        }
      );
