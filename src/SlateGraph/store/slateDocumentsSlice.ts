...
const slateDocumentsSlice = createSlice({
    ...
    extraReducers(builder) {
        builder
            .addCase(createDoc, (state, { payload: { docName, doc, withBackref, createdDate } }) => {
                const referencesSet = getReferencesFromNodes(doc)
                const references = Array.from(referencesSet);

                Object.keys(state).forEach(docKey => {
                    if (referencesSet.has(docKey) && !state[docKey].backReferences.includes(docName)) {
            ...
            .addCase(updateDoc, (state, { payload: { docName, newDoc, updatedDate } }) => {
                const referencesSet = getReferencesFromNodes(newDoc);
                const references = Array.from(referencesSet);
                const prevReferences = state[docName].references;
                const referencesDropped = prevReferences.filter(x => !referencesSet.has(x));

                Object.keys(state).forEach(docKey => {
                    const backReferences = state[docKey].backReferences.filter(bref => !referencesDropped.includes(bref));
