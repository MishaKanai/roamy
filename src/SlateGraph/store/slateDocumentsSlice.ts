    ...
    extraReducers(builder) {
        builder
            .addCase(
                ...
                (
                    state,
                    { payload: { docName, doc, withBackref, createdDate } }
                ) => {
            ...
            .addCase(
                ...
                (state, { payload: { docName, newDoc, updatedDate } }) => {
                    ...
                    Object.keys(state).forEach((docKey) => {
                        const backReferences = referencesDropped.includes(
                            docKey
                        )
                            ? state[docKey].backReferences.filter(
                                  (bref) => bref !== docName
                              )
                        ...
                        state[docKey].backReferencesHash =
                            hashSum(backReferences);
                    });
