import React, {
  ...
  const ImageIdLink = (props: { imageId: string; children: (url: string) => JSX.Element }) => {
    const image = useAppSelector(state => state.uploadedFiles[props.imageId]);
    return props.children(image?.fileData?.dataURL);
  }
  ...
  const InlineImage = ({ attributes, children, element: _element }: RenderElementProps) => {
    ...
    const [newImageState, setNewimagestate] = useState<string>('');
    const [newImageSize, setNewImageSize] = useState<number>(0);
    useEffect(() => {
      const size = getImageSize(newImageState)
    ...
    return (
      ...
      >
        <div
          ...
          `}
        >
          {newImageSize && newImageState && <p>{newImageSize} kb</p>}
          {newImageState && <img
              alt="user uploaded"
              src={newImageState}
              className={imageClassName}
            />}
          {dataUrl && getImageSize(dataUrl)}{' kb'}
            ...
            <div style={{ position: 'relative' }}>
              <IconButton
                ...
                onClick={(e) => {
                  ...
                  loadGif(dataURLtoFile(dataUrl, '1.gif')).then(base64 => {
                    console.log({
                      base64
                    })
                    setNewimagestate(base64)
                  })
                  // gif.on('finished', (blob: any, data: any) => {
                  //   console.log({
                  //     blob,
                  //     data
                  //   })
                  ...
                  // gifFrames({ url: dataUrl, frames: 'all', outputType: 'canvas', cumulative: true }).then(function (frameData: any) {
                  //   let i = 0
                  //   console.log({
                  //     frameData
                  //   })
                  ...
                  //     // if (i++ % 2 === 0) {
                  //       const frameDelay = (frame as any).frameInfo.delay;
                  //       const canvas = (frame as any).getImage();
                  ...
                  // image.onload = () => {
                  //   console.log(image)
                  //   console.log('animations', image.getAnimations())
                  //   gif.addFrame(image, { delay: 500, copy: true});
                  //   gif.render();
                  // }
                  // console.log({
                  //   element
