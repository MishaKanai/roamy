...
const createFilesForDrawingSelector = <
  T extends {
    drawings: {
      [drawingName: string]: {
...
>() =>
  createSelector(
    (state: T, drawingName: string) =>
      state.drawings[drawingName]?.drawing?.filesIds,
    (state: T) => state.files.uploadedFiles,
    (filesIds, uploadedFiles) => {
      return (
        filesIds?.reduce((prev, currId) => {
          const uploadedFile = uploadedFiles[currId];
          if (!uploadedFile) {
            return prev;
          }
          prev[currId] = uploadedFile.fileData;
          return prev;
        }, {} as BinaryFiles) ?? null
      );
    }
