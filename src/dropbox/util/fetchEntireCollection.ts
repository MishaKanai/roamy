...
const fetchDataFromCollectionAndCompose = async (
  dbx: Dropbox,
  indexFilePath: string,
  indexFileRevision?: string
) => {
  const { data, rev } = await loadFileJSON<IndexFileStructure>(
    dbx,
    indexFilePath,
    indexFileRevision
  );
  ...
  const documentResults = await Promise.allSettled([
    ...(data.uploadedFiles?.map((fid) =>
      loadFileJSON<BinaryFileData>(
        dbx,
        folderName + "file_" + fid + ".json"
      ).then((res) => ({
        ...res,
        type: "file" as const,
      }))
    ) ?? []),
    ...documentFileNames.map((dfn) =>
      loadFileJSON<SlateDocument>(
        dbx,
        folderName + "doc_" + dfn + ".json",
        data.documents[dfn].rev
      ).then((res) => ({
        ...res,
        type: "doc" as const,
      }))
    ),
    ...drawingFileNames.map((dfn) =>
      loadFileJSON<DrawingDocument>(
        dbx,
        folderName + "drawing_" + dfn + ".json",
        data.drawings[dfn].rev
      ).then((res) => ({
        ...res,
